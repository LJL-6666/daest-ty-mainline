...
![总结](results/figures/summary/00_总结.png)

<sub>A 素材九类全量基线　B 自评八类（虚线＝A 栏素材九类参照）　C 同人 matched51（灰线＝同一被试配对）　D 迁移（虚线＝目标 scratch）。13 个结果各出现一次，虚线为已在其他栏出现过的参照值。</sub>

**每个实验的逐被试准确率与混淆矩阵**见 [`docs/02_实验索引.md`](docs/02_实验索引.md)（16 张图已内嵌）。
