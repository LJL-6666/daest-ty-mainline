...
import torch
from .io_utils import load_EEG_data, load_processed_SEEDV_NEW_data, load_processed_FACED_NEW_data, save_sliced_data, get_ty6_save_base
import os
import fcntl
import numpy as np
import random
...
def _ensure_sliced_cache(sliced_data_dir, load_dir, cfg):
    """Build sliced cache at most once; safe under multi-process hydra collisions."""
    os.makedirs(sliced_data_dir, exist_ok=True)
    saved_path = os.path.join(sliced_data_dir, 'saved.npy')
    lock_path = os.path.join(sliced_data_dir, '.build.lock')
    with open(lock_path, 'w') as lock_f:
        ...
        try:
            ...
            data, onesub_labels, n_samples_onesub, n_samples_sessions = load_EEG_data(load_dir, cfg)
            save_sliced_data(
                sliced_data_dir=sliced_data_dir,
                data=data,
                onesub_labels=onesub_labels,
                n_samples_onesub=n_samples_onesub,
...
class EEG_Dataset(Dataset):
    def __init__(self, cfg, train_subs=None, val_subs=None, sliced=True, mods=None):
        # TY6: data_dir 为含 pkl 的目录，无 processed_data；save 可用 work_dir 避免写入预处理目录
        if getattr(cfg, 'dataset_name', None) == 'TY6':
            self.load_dir = cfg.data_dir
            self.save_dir = os.path.join(get_ty6_save_base(cfg), 'sliced_data')
        else:
        ...
        if not sliced:
            _ensure_sliced_cache(self.sliced_data_dir, self.load_dir, cfg)
