...
class MLPModel(pl.LightningModule):
    ...
    def _acc_ignore_invalid(self, logits, labels):
        mask = labels != -1
        if not bool(mask.any()):
            return torch.tensor(0.0, device=logits.device)
        top1 = self.metric(logits[mask], labels[mask], topk=(1,))
        return top1[0]
    ...
    def training_step(self, batch, batch_idx):
        ...
        top1 = self._acc_ignore_invalid(logits, labels)
        self.log_dict({'mlp/train/loss': loss, 'mlp/train/acc': top1}, on_step=False, on_epoch=True, prog_bar=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.criterion(logits, labels)
        top1 = self._acc_ignore_invalid(logits, labels)
        self.log_dict({'mlp/val/loss': loss, 'mlp/val/acc': top1}, on_epoch=True, prog_bar=True)
        return loss
