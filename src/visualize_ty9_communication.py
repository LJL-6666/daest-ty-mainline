...
# -*- coding: utf-8 -*-
"""
TY9_communication 5 折分类结果可视化：被试准确率条形图 + 9 类混淆矩阵。
口径：118 人 / 28 视频 / 9 类 / ME；默认读取 TY9_communication_ME_cp，不覆盖其他结果。

用法（在 FACED-base 目录下）:
  python visualize_ty9_communication.py
  python visualize_ty9_communication.py --feat_dir ... --cp_dir ... --out_dir ... --run 1
"""
...
def _guess_default_base_roots():
    roots = []
    roots.append(os.path.join('runs', 'TY9_communication_ME_cp', 'runs'))
    # 若以后有并行/备份目录，按修改时间兜底
    extra = glob.glob(os.path.join('runs', 'TY9_communication_ME_cp*', 'runs'))
    ...
    if feat_dir and cp_dir:
        ...
        if feat_dir is None or cp_dir is None:
            raise FileNotFoundError(
                'Cannot auto-detect TY9_communication result paths. Please pass --feat_dir and --cp_dir explicitly.\n'
                'Expected under runs/TY9_communication_ME_cp/runs/{TY9_communication/ext_fea/fea_r*, cp/TY9_communication/r*}'
            )
    if out_dir is None:
...
def plot_subject_accuracy_bar(acc_per_sub, out_path, title=None):
    ...
    ax.set_title(title or 'TY9_communication 5-Fold Classification - Subject Accuracy')
    plt.tight_layout()
...
def plot_confusion_matrix(all_pred, all_true, out_path, csv_path=None, title=None):
    ...
    ax.set_title(title or 'TY9_communication 5-Fold Classification - Confusion Matrix (Row Norm.)')
    plt.tight_layout()
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='TY9_communication 5-Fold 9-class visualization')
    ...
    parser.add_argument('--n_subs', type=int, default=None, help='override N_SUBS (default 118)')
    parser.add_argument('--save_name', type=str, default=None, help='checkpoint subfolder name')
    parser.add_argument('--title_prefix', type=str, default='TY9_communication')
