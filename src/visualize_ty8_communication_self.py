...
# -*- coding: utf-8 -*-
"""
TY8_communication_self 5 折分类结果可视化：被试准确率条形图 + 8 类混淆矩阵。
口径：118 人 / 28 视频 / 自评 argmax 8 类 / ME；读取 TY8_communication_self_ME_cp。

用法（在 FACED-base 目录下）:
  python visualize_ty8_communication_self.py
  python visualize_ty8_communication_self.py --feat_dir ... --cp_dir ... --out_dir ... --run 1
"""
...
def _guess_default_base_roots():
    roots = []
    roots.append(os.path.join('runs', 'TY8_communication_self_ME_cp', 'runs'))
    extra = glob.glob(os.path.join('runs', 'TY8_communication_self_ME_cp*', 'runs'))
    ...
    if feat_dir is None or cp_dir is None:
        ...
        if feat_dir is None or cp_dir is None:
            raise FileNotFoundError(
                'Cannot locate TY8_communication_self features/checkpoints. '
                'Expected under runs/TY8_communication_self_ME_cp/runs/{TY8_communication_self/ext_fea/fea_r*, cp/TY8_communication_self/r*}'
            )
    if out_dir is None:
...
def plot_subject_accuracy_bar(acc_per_sub, out_path):
    ...
    ax.set_title('TY8_communication_self 5-Fold Classification - Subject Accuracy')
    plt.tight_layout()
...
def plot_confusion_matrix(all_pred, all_true, out_path, csv_path=None):
    ...
    ax.set_title('TY8_communication_self 5-Fold Classification - Confusion Matrix (Row Norm.)')
    plt.tight_layout()
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='TY8_communication_self 5-Fold 8-class visualization')
    parser.add_argument('--feat_dir', type=str, default=None)
    parser.add_argument('--cp_dir', type=str, default=None, help='Checkpoint root containing TY8_communication_self/r*/')
    ...
    plot_subject_accuracy_bar(
        acc_per_sub, os.path.join(args.out_dir, f'ty8_communication_self_5folds_subject_accuracy_r{args.run}.png')
    )
    plot_confusion_matrix(
        all_pred,
        all_true,
        os.path.join(args.out_dir, f'ty8_communication_self_5folds_cls8_confusion_r{args.run}.png'),
        csv_path=os.path.join(args.out_dir, f'ty8_communication_self_5folds_cls8_confusion_matrix_r{args.run}.csv'),
    )
